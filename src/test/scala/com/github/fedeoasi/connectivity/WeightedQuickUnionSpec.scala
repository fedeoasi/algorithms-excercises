...
class WeightedQuickUnionSpec extends FunSpec with ShouldMatchers {
  describe("Weighted Quick Union") {
    it("should initialize to an array where the values are the indexes") {
      val wqu = new WeightedQuickUnion(5)
    ...
    it("should connect two subtrees") {
      ...
      wqu.printArray() should be("1 1 0 1 1")
    }

    it("should union and print") {
      val wqu = new WeightedQuickUnion(10)
    ...
    def unionAndPrint(wqu: WeightedQuickUnion, p: Int, q: Int) {
      wqu.union(p, q)
